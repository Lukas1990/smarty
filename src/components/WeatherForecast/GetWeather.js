...
function WeatherForecast(props) {
  ...
  const getWeatherData = () => {
    ...
    const latitude = select.current.selectedOptions[0].dataset.latitude
    const longitude = select.current.selectedOptions[0].dataset.longitude
    ...
    axios.get('https://api.open-meteo.com/v1/forecast?latitude='+latitude+'&longitude='+longitude+'&daily=weathercode,temperature_2m_max,rain_sum,snowfall_sum,windspeed_10m_max&timezone=Europe%2FBerlin&start_date='+today+'&end_date='+futureDate)
  ...
  <div>
    ...
    <button type="submit" className="btn" onClick={getWeatherData}>{phrase["Find out"]}</button>
  </div>
  );
}
